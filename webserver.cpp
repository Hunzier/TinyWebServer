...
void WebServer::eventListen()
{
    ...
    //m_OPT_LINGER为是否优雅下线, 即是四次挥手,  通过setsockopt设置listenfd
    if (0 == m_OPT_LINGER)
    {
...
//并对新的定时器在链表上的位置进行调整
void WebServer::adjust_timer(int fd)
{
    time_t cur = time(NULL);
    utils.m_timer_heap.update(fd, cur + 3 * TIMESLOT);
